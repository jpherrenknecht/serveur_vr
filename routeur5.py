...
def f_isochrone(pt_init_cplx, temps_initial_iso):
    ...
    #tri de la liste de tous les  points obtenus au nombre de i*j
...
instant        = tig                                #   pour utiliser le grib sinon time.time()
...
i = 0
for n in (route):         # coordonneees des differents points du chemin sous forme nparray complexe
    chemin[i] = isochrone[n][0] + isochrone[n][1] * 1j
    i += 1
chemin[i] = A      # le bout du chemin c'estl'arrivee
